...
export function decodeCursor(cursor: string | undefined): { key: string; id: string } | null {
  ...
  try {
    const [key, id] = CursorShape.parse(JSON.parse(Buffer.from(cursor, 'base64url').toString('utf8')));
    return { key, id };
  } catch {
    throw Errors.badRequest('INVALID_CURSOR', 'The page cursor is invalid. Start again from the first page.');
  }
}
