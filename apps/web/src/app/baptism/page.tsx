...
export const metadata: Metadata = {
  title: 'Baptism',
  description: 'Thinking about being baptised? Find out what to expect and ask your branch to get in touch.',
  alternates: { canonical: '/baptism' },
};

const STEPS = [
  { title: 'Let us know', text: 'Send the short form on this page. It goes straight to the branch you choose.' },
  { title: 'We get in touch', text: 'Someone from the branch contacts you to talk, answer questions and pray with you.' },
  { title: 'Preparation', text: 'You meet with a minister to prepare, at a pace that suits you.' },
  { title: 'Baptism service', text: 'You are baptised at a service, surrounded by your church family.' },
];
...
export default async function BaptismPage({ searchParams }: PageProps<'/baptism'>) {
  ...
  const [services, stories] = await Promise.all([
    client.GET('/api/v1/events', { params: { query: { category: 'BAPTISM', when: 'upcoming', limit: 4 } }, fetch }).then(unwrap),
    client.GET('/api/v1/content', { params: { query: { types: 'baptism', limit: 3 } }, fetch }).then(unwrap),
  ]);
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,28rem)] lg:gap-12">
          <div className="flex min-w-0 flex-col gap-10">
            ...
            <section aria-labelledby="services-heading" className="flex flex-col gap-4">
              <SectionHeading id="services-heading" icon={<CalendarDays />} action={{ href: '/events?category=baptism', label: 'All baptism services' }}>
              ...
              {services.items.length > 0 ? (
                ...
                <p className="text-sm text-muted">No baptism services are scheduled right now. Your branch will let you know the next date.</p>
              )}
            </section>

            {stories.items.length > 0 ? (
              <section aria-labelledby="stories-heading" className="flex flex-col gap-4">
                <SectionHeading id="stories-heading" action={{ href: '/feed?types=baptism', label: 'More' }}>
                  Recent celebrations
                </SectionHeading>
          ...
          <section aria-labelledby="request-heading" className="lg:sticky lg:top-24 lg:self-start">
            <Card className="flex flex-col gap-5 p-5 sm:p-6">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-muted">There is no obligation. We will simply get in touch.</p>
              </div>
              {organization.baptismRequestsEnabled && branches.length > 0 ? (
                <BaptismRequestForm branches={branches.map((b) => ({ slug: b.slug, name: b.name }))} defaultBranch={defaultBranch} />
              ) : (
                <Alert tone="info" title="Online requests are closed for now">
                  Please speak to someone at your branch after a service. <a href="/branches">Find your branch</a>.
                </Alert>
              )}
            </Card>
          </section>
