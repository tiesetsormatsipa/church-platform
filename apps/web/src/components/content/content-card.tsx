...
import { formatCalendarDate, formatDuration, formatEventTiming, formatRelative } from '@/lib/format';
 ...
 */
export function ContentCard({ item, now, headingLevel = 3 }: { item: ContentSummary; now?: Date; headingLevel?: 2 | 3 }) {
  ...
  return (
    ...
    >
      {item.event ? <DateTile iso={item.event.startsAt} timeZone={item.event.timezone} cancelled={cancelled} /> : null}
      <div className="flex min-w-0 flex-1 flex-col gap-2">
        ...
        <Heading className={cn('text-lg leading-snug font-semibold text-foreground sm:text-xl', cancelled && 'line-through decoration-1')}>
          <Link href={item.path} className="after:absolute after:inset-0 after:rounded-xl focus-visible:outline-none">
            {item.title}
          </Link>
        </Heading>
        {item.summary ? <p className="line-clamp-3 text-sm leading-relaxed text-muted">{item.summary}</p> : null}
...
function Meta({ item, now }: { item: ContentSummary; now?: Date }) {
  ...
  if (item.event) {
    ...
    parts.push(
      <span key="speaker" className="inline-flex items-center gap-1.5">
        ...
        {[item.sermon.speakerName, formatCalendarDate(item.sermon.preachedOn)].filter(Boolean).join(' · ')}
      </span>,
    );
  ...
  return <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted">{parts}</div>;
}
