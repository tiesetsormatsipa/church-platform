...
export function usePagedItems(endpoint: PagedEndpoint, query: Record<string, string | undefined>, initialCursor: string | null) {
  ...
  async function loadMore() {
    ...
    setStatus('loading');
    try {
      const params = new URLSearchParams({ ...Object.fromEntries(Object.entries(query).filter(([, v]) => v)), cursor });
