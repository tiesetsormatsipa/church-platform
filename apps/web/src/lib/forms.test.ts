...
describe('applyApiError', () => {
  it('maps field errors onto the form', () => {
    ...
    expect(applyApiError(error, setError, ['email', 'fullName'])).toBe(
  ...
  it('keeps the API message when an error has no matching field', () => {
    const setError = vi.fn();
    const error = new ApiError(400, {
      detail: 'That branch could not be found.',
      errors: [{ path: 'branch', message: 'Unknown' }],
    });
  ...
  it('explains rate limits and network failures', () => {
    expect(
      applyApiError(new ApiError(429, { detail: 'Try again in 5 minutes.' }), vi.fn(), []),
    ).toBe('Try again in 5 minutes.');
    expect(applyApiError(new ApiError(429, undefined), vi.fn(), [])).toMatch(
      /Something went wrong|too many times/,
    );
