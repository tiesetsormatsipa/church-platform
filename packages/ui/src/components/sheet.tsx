...
const SWIPE: Record<Side, 'down' | 'right' | 'left'> = { bottom: 'down', right: 'right', left: 'left' };

export function Sheet({ side = 'bottom', ...props }: React.ComponentProps<typeof Drawer.Root> & { side?: Side }) {
 ...
 */
export function SheetContent({ side = 'bottom', title, description, hideTitle, className, children }: SheetContentProps) {
  const position = {
  ...
  const panel = {
    ...
    left:
      'h-full w-[min(22rem,calc(100vw-3rem))] border-r [transform:translateX(var(--drawer-swipe-movement-x))] data-starting-style:[transform:translateX(-100%)] data-ending-style:[transform:translateX(-100%)]',
  }[side];
  return (
    <Drawer.Portal>
      ...
      <Drawer.Viewport className={cn('fixed inset-0 z-50 flex', position)}>
        <Drawer.Popup
          className={cn(
            ...
            'transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] data-swiping:select-none data-swiping:duration-0',
            panel,
            className,
          )}
        >
          {side === 'bottom' ? (
            <div aria-hidden="true" className="mx-auto mb-3 h-1.5 w-10 shrink-0 rounded-full bg-border-strong" />
          ) : null}
          <Drawer.Content className="flex flex-col">
            <div className={cn('mb-4 flex items-start justify-between gap-4', side !== 'bottom' && 'pt-3', hideTitle && 'sr-only')}>
              <div className="flex flex-col gap-1">
                <Drawer.Title className="text-lg font-semibold">{title}</Drawer.Title>
                {description ? <Drawer.Description className="text-sm text-muted">{description}</Drawer.Description> : null}
              </div>
              <Drawer.Close
                aria-label="Close"
